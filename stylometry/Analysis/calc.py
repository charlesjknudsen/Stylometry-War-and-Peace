...
def averageVector(vector):
	avgValues = []
	avgValue = 0
	for j in range(0, len(vector[0])): #all feature lists within a vector have the same length, so selecting vector[0] doesn't make a difference
		for i in range(0, len(vector)):
			avgValue += vector[i][j]
		avgValues.append(avgValue/len(vector))
		avgValue = 0
...
#assigns a different color to each vector's chunks. 
def assignColors(vectorList):
	colors = ["r.", "g.", "b.", "y.", "c.", "m."]
	#colors = ["r.", "", "", "y.", "", ""]
	returnVector = []
	
	for i, subVector in enumerate(vectorList):
		for j in range(len(subVector)):
			returnVector.append(colors[i])
...
def Analysis(vector, colorArrangement, K=2):
	...
	"""kmeans = KMeans(n_clusters=K) #n_jobs does not exist anymore, the maximum amount of parallel sqeuences are always run.
	...
	for i in range(len(components)):
		...
		plt.plot(components[i][0], components[i][1], colorArrangement[i], markersize=10)
...
if __name__ == '__main__':
	...
	#documents = ["WAP/briggs.txt", "WAP/dole.txt", "WAP/dunnigan.txt", "WAP/garnett.txt", "WAP/maude.txt", "WAP/volokhonsky.txt"]
	#documents = ["WAP/briggs.txt", "WAP/volokhonsky.txt"]
	#documents = ["Calc/10.txt", "Calc/10.txt"]
	#documents = ["Calc/100.txt", "Calc/100.txt"]
	#documents = ["Calc/1,000.txt", "Calc/1,000.txt"]
	#documents = ["Calc/4,000.txt", "Calc/4,000.txt"]
	#documents = ["Calc/10,000.txt", "Calc/10,000.txt"]
	vector = []
	vectorList = []
	
	for document in documents:
		vector += FeatureExtration(open(document, encoding="utf8").read(), winSize=1, step=1) #change to 50/10 (winsize, step)
		vectorList.append(FeatureExtration(open(document, encoding="utf8").read(), winSize=1, step=1)) #change to 50/10 (winsize, step)
	...
	n = 5
	result = timeit.timeit(stmt='Analysis(vector, colorArrangement)', globals=globals(), number=n)
	print(result)
	
	#Create Stylometric Features Table:
	...
	"""data = []
	headers = ["Translation", "Average Word Length", "Average Sentence Length By Chunk", "Average Sentence Length by Word", "Average Syllable Per Word", "Special Character Count", "Punctuation Count", "Functional Words Count", "Type Token Ratio", "Hapax Legemena", "Honore Measure R", "Hapax Dislegemena", "Sichele's Measure S", "Yules Characteristic K", "Simpson's Index", "Brunet's Measure W", "Shannon Entropy", "Flesch Reading Ease", "Flesch-Kincaid Grade Level", "Dale Chall Readability Formula", "Gunning-Fog Index"]
	for i in range(0,len(documents)):
		data.append([documents[i]] + averageVector(FeatureExtration(open(documents[i], encoding="utf8").read(), winSize=50, step=50)))
		
	stylometricTable = tabulate(data, headers=headers, tablefmt="html")
	print(stylometricTable)"""
